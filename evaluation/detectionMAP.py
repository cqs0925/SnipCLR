...
def getLocMAP(seg_preds, th, annotation_path, args, multi=False, factor=1.0, knn=False):
    try:
...
def getSingleStreamDetectionMAP(
    vid_preds, frm_preds, vid_lens, annotation_path, args, multi=False, factor=1.0, knn= False
):
    ...
    for iou in iou_list:
        ...
        dmap_list.append(
            getLocMAP(seg, iou, annotation_path, args, multi=multi, factor=factor, knn = knn)
        )
