...
class Model2(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, hidden_dim, num_class, graph_args,
        ...
        self.pool = nn.AdaptiveAvgPool1d(self.t_grid)
        self.mlp2 = nn.Sequential(
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(hidden_dim, num_class, kernel_size=1)
        )
    ...
    def forward(self, x, DC=False):
        ...
        # same till here!!
        # prediction
        if self.no_pretrain:  # always false, so always pretrain!!
            x = F.avg_pool2d(x, x.size()[2:])
            x = x.view(N, M, -1).mean(dim=1)
            x = self.fc(x)    #close if do tsne
            x = x.view(x.size(0), -1)
            ...
            if not DC: # it has DC in pretraining, but here we didn't add denseCL loss to it for now.
                # -------- MLP 路径（全局特征） --------
                avgpooled_x = F.avg_pool2d(x, x.size()[2:])
                avgpooled_x = avgpooled_x.view(N, M, -1).mean(dim=1)
                x1 = self.fc1(avgpooled_x)
                x2 = self.fc2(avgpooled_x)
                x1 = x1.view(x1.size(0), -1)
                x2 = x2.view(x2.size(0), -1)
                # -------- Conv 路径（密集空间特征） --------
                ...
                x = self.pool(x)
                backbone_feat = x
                x = self.mlp2(x)  # (B, C, 10) → (B, out, 10)
                dense_global = self.avgpool2(x).view(x.size(0), -1)  # → (B, out)
                dense_x = x.view(x.size(0), x.size(1), -1)  # → (B, out, 10)

                return x1, x2, dense_x, dense_global, backbone_feat
            
            else: # it has DC in pretraining, but here we didn't add denseCL loss to it for now.
                x = self.part(x)  # [NM,C,T,5] #inter-part
                x = self.relation(x) #[NM,C,T,5]
                x = F.avg_pool2d(x, x.size()[2:])
                x = x.view(N, M, -1).mean(dim=1)
