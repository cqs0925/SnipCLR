    ...
    def train(self, epoch):
        ...
        for [data1, data2, data3], label in loader:
            ...
            if epoch <= self.arg.mining_epoch:
                ...
                logits1, logits2, labels_A, logits_M, labels_M, logits_A1_dc, labels_dc,logits_dense, labels_dense, loss_pro = self.model(data1, data2, data3) 
                if hasattr(self.model, 'module'):
                    self.model.module.update_ptr(logits1.size(0))
                else:
                    self.model.update_ptr(logits1.size(0))
                labels_M = labels_M.unsqueeze(dim=-1)
                ...
                loss = loss1 + loss2 + loss3  + loss4 + 1.5* loss_dense  #+  loss_pro
            else:
                logits1, logits2, pos_mask, logits_M, labels_M, logits_A1_dc, labels_dc, logits_dense, pos_mask_dense,loss_pro = self.model(data1, data2, data3, nnm=True, topk=self.arg.topk) 
                if hasattr(self.model, 'module'):
                    self.model.module.update_ptr(logits1.size(0))
                else:
                    self.model.update_ptr(logits1.size(0))
                ...
                loss4 = -torch.mean(torch.sum(torch.log(logits_A1_dc) * labels_dc, dim=1))
                # loss_dense = self.loss(logits_dense, labels_dense)
                loss_dense = - (F.log_softmax(logits_dense, dim=1) * pos_mask_dense).sum(1) / pos_mask_dense.sum(1)
