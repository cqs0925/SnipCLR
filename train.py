...
class Processor:
    ...
    def __init__(self, arg):
        ...
        self.my_logger.set_names(["Step"] + [f"map_0.{i}" for i in range(1, 6)] + ["avg"])
    ...
    def train(self, epoch, wb_dict, save_model=False):
        ...
        for batch_idx, (data, label, target, mask, index, soft_label) in enumerate(
            ...
            one_hot_ground_truth = F.one_hot(ground_truth_flat, num_classes=2)
            ''' into one hot'''
    ...
    def eval(
        ...
        for ln in loader_name:
            ...
            self.my_logger.append([epoch + 1] + dmap + [np.mean(dmap)])
    ...
    def start(self):
        ...
        if self.arg.phase == "train":
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if epoch%1==0:
                # Eval. on val set
                    wb_dict,map = self.eval(epoch, wb_dict, loader_name=["test"])
                    if map>best_map:
                        best_map = map
                        best_epoch = epoch
                    # Log stats. for this epoch
                    print("Epoch: {0}\nMetrics: {1}".format(epoch, wb_dict))
            self.print_log("\tBest mAP: {:.2f} in epoch {}".format(best_map, best_epoch))
