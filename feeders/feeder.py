...
class Feeder_semi(Dataset):
    def __init__(
        ...
        total_len = len(self.data["X"])
        sample_size = max(1, int(total_len * use_fraction))
        random.seed(seed)
        indices = random.sample(range(total_len), sample_size)
        indices.sort()  # Optional, for consistent ordering

        self.data["X"] = [self.data["X"][i] for i in indices]
        self.data["Y"] = [self.data["Y"][i] for i in indices]
        self.data["L"] = [self.data["L"][i] for i in indices]
        '''semi'''
